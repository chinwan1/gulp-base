var storeDatafd924 = {
    ...
    hasStateFlowPurposeExpired() {
      return storeDatafd924.state.stateFlow === storeDatafd924.statesFlow.purposeExpired;
    },
    ...
    sendHistoryToDatabase() {
        ...
        for (let i = 0; i < storeDatafd924.state.purposes.length; i++) {
            if (storeDatafd924.state.purposes[i].hasAllow) {
                purposeBody[j] = { "ConsentCookiePurposeID": storeDatafd924.state.purposes[i].id, "HasAllow": true }
                j++;
            }
        }
        let body = {
            ...
            "IPAddress": `${{ipAddress}}`,
            "Referenceid": storeDatafd924.state.referenceId,
            "CreateDate": dateNow,
            "ConsentCookieID": storeDatafd924.state.consentCookieID,
    ...
    watchEventModalPreview() {
        document.addEventListener("EventPreview", (e) => {
            ...
            if (e.detail.action === "OPEN_MODAL" && !item) {
                ...
                document
                    .querySelector(`.fd924-cookie-consent`)
                ...
                if (data.consentCookieID) storeDatafd924.state.consentCookieID = data.consentCookieID;
    ...
    loadDataFromPersistence() {
      storeDatafd924.state.purposesExpired = [];
        var storeData = storeDatafd924.getCookieByName(`storeDatafd924-${storeDatafd924.state.consentCookieID}`);
        if (!storeData) return { emptyData: true };
        var item = JSON.parse(storeData);
        if (!item) return { emptyData: true };
        let currentDate = new Date();
        item.purposes= item.purposes.map((storePurpose) => {
            let found = storeDatafd924.state.purposes.find(sitem => sitem.id === storePurpose.id);
            found.expired = storePurpose.expired;
            found.hasAllow = storePurpose.hasAllow;
            if(new Date(storePurpose.expired) < currentDate && storePurpose.hasAllow) {
              storeDatafd924.changeStateFlowToStateFlowExpired();
              found.hasAllow = false;
              storeDatafd924.state.purposesExpired.push(found);
             }
            return found;
        })
        return {
    ...
    setInitState(input) {
        ...
        storeDatafd924.state.referenceId = input.referenceId;
        ...
        storeDatafd924.state.consentCookieID = input.consentCookieID;
    ...
    changeStateFlowToStateFlowExpired() {
        storeDatafd924.state.stateFlow = storeDatafd924.statesFlow.purposeExpired;
    },
    changeStateFlowExpiredToStateFlowNormal() {
      storeDatafd924.state.stateFlow = 0;
      storeDatafd924.state.hasStateFlowPurposeExpired = [];
    },
    ...
    saveState() {
      if(this.hasStateFlowPurposeExpired()) {
        storeDatafd924.state.purposes = this.mappingPurposesExpired();
        storeDatafd924.changeStateFlowExpiredToStateFlowNormal();
      }
      document.cookie = `storeDatafd924-${storeDatafd924.state.consentCookieID}=${JSON.stringify({
      purposes: storeDatafd924.state.purposes.map((item) => {
      let [amount,unit]= storeDatafd924.processKeepAliveDataToForm(item.keepAliveData);
      if(item.hasAllow) { expired = storeDatafd924.calculateDate(amount,unit); }
      ...
      cookieID: storeDatafd924.state.consentCookieID,
    })}; path=/`;
      storeDatafd924.sendHistoryToDatabase();
    },
    mappingPurposesExpired() {
      return storeDatafd924.state.purposes.map((item) => {
        let codeItem = item.code;
        let fitem = storeDatafd924.state.purposesExpired.find((pe) => {
...
var CSent = {
    inititail: (options) => {
        ...
        if (storeDatafd924.getCookieByName(`CONSENT-${storeDatafd924.state.consentCookieID}`)) {
            if(storeDatafd924.hasStateFlowPurposeExpired()) {
    ...
    exportFunction()  {
      var consentCookieID = storeDatafd924.state.consentCookieID;
      var storeData = storeDatafd924.getCookieByName(`storeDatafd924-${consentCookieID}`);
      if (!storeData) return storeDatafd924.mappingExternalCookie(consentCookieID,storeDatafd924.state.purposes);
      var item = JSON.parse(storeData);
      if (!item) return storeDatafd924.mappingExternalCookie(consentCookieID,storeDatafd924.state.purposes);
      return storeDatafd924.mappingExternalCookie(consentCookieID,item.purposes)
    },
    ...
    onAccept() {
      try {
        var d = new Date();
            ...
            document.cookie = `CONSENT-${storeDatafd924.state.consentCookieID}=` + cvalue + ";" + expires + ";path=/";
...
const cookieConsentNavigation = `fd924-cookie-consent-navigation`;
class CookieConsentNavigation extends HTMLElement {
    ...
    renderHtml() {
        this.innerHTML = /*html*/`
				<div class="fd924-cookie-consent-navigation">
          <fd924-cookie-consent-navigation-item></fd924-cookie-consent-navigation-item>
				</div>
		`
    }
...
const cookieConsentNavigationItem = `fd924-cookie-consent-navigation-item`;
class CookieConsentNavigationItem extends HTMLElement {
    ...
    renderHtml() {
        this.innerHTML = /*html*/`
				<div class="fd924-cookie-consent-navigation-item">
        111
				</div>
		`
    }
...
const cookieConsentContent = `fd924-cookie-consent-content`;
...
const cookieConsent = `fd924-cookie-consent`;
class CookieConsent extends HTMLElement {
    ...
    connectedCallback() {
        ...
        if (!localStore.emptyData) {
            ...
            storeDatafd924.state.purposes = storeDatafd924.state.purposes.map((item) => {
                item.hasAllow = false;
                return item;
            })
        }
    ...
    renderHtml() {
        ...
        this.innerHTML = /*html*/ `
		<div class="fd924-cookie-consent">
			<div class="fd924-model-cookie fd924-modal">
				<div class="fd924-model-content">
					<div class="fd924-model-header">
              <div class="fd924-box-left">
                <div class="fd924-model-header-logo">
                  1ddd
                </div>
              </div>
              <div class="fd924-box-right">
                <div class="fd924-model-header-content">
            ...
            <div  class="fd924-box-left">
              <fd924-cookie-consent-navigation> <fd924-cookie-consent-navigation>
            </div>
            <div  class="fd924-box-right">
              <fd924-cookie-consent-content></fd924-cookie-consent-content>
            </div>
					</div>
						<div class="fd924-model-footer">
							...
							<div class="fd924-box-powered-by">
								<a href="https://www.thewiseworks.com/" >
									<span class="fd924-powerd-by">${storeDatafd924.state.labeltextPowerText || "Powered by"
            }</span> <span class="fd924-powerd-link">${storeDatafd924.state.powerByText || "wisework"
            }<span>
								</a>
							</div>
						</div>
        ...
        this.querySelector(`.fd924-cookie-consent`).classList.add("fd924-anime");
        this.querySelector(".fd924-text-title-header").style.color =
            `${ci.modalTitleColor}`;
        if(storeDatafd924.hasStateFlowPurposeExpired()) {
    ...
    onToggleModal() {
        ...
        if (!this.state.hasOpen) {
            ...
            this.querySelector(`.fd924-cookie-consent`).classList.add("fd924-out");
            setTimeout(() => {
                this.remove();
            }, 500);
        }
    ...
    registerEvent() {
        this.querySelector(`.fd924-model-icon-x`).addEventListener(
            "click",
...
const iconCookieConsent = `fd924-icon-cookie`;
