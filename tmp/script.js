...
var storeDatafd924 = {
  ...
  watchEventModalPreview: function watchEventModalPreview() {
    document.addEventListener("EventPreview", function (e) {
      ...
      if (e.detail.action === "OPEN_MODAL" && !item) {
      ...
      } else if (e.detail.action === "INIT_TOKEM") {
        ...
        if (item) {
          ...
          setTimeout(function () {
            item.remove();
            if (!itemPrivacy) CSent.renderFooterPrivacy({});
          }, 500);
        } else if (itemCookie) {
          itemCookie.remove();
...
var IconCookieCosent = /*#__PURE__*/function (_HTMLElement5) {
  ...
  _createClass(IconCookieCosent, [{
    ...
    value: function renderHtml() {
      ...
      this.querySelector('.fd924-icon-cookie').style.background = storeDatafd924.state.ciColor.iconCookie.bgColor;
    }
  }, {
    key: "registerEvent",
    value: function registerEvent() {
